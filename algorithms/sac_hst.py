...
class SAC(object):
    ...
    def __init__(self, nagents, obs_shape,
        ...
        #Note: need to instatiate agents with message passed from other agents
    ...
    def update_critic(self, sample, soft=True, logger=None, intr_rews=None,
        ...
        #Changess from Aowabin to The q-function based on
        # nagents = np.arange(self.nagents).tolist()
        #
        # if "selected_agent_ids" in kwargs:
        #     select_ais = kwargs["selected_agent_ids"]
        #     if kwargs["selected_agent_ids"] is not None:
        #         nagents = [nagents[ai-1] for ai in select_ais] #-1 to match IDs with indices
        #         next_qs  = [next_qs[ai-1] for ai in select_ais]
        #         next_log_pis = [next_log_pis[ai-1] for ai in select_ais]
        #         pred_qs = [pred_qs[ai-1] for ai in select_ais]

        #troubleshoot
        #add selected_ids
        if "selected_agent_ids" not in kwargs:
            select_ais = np.arange(self.nagents).tolist()
        elif kwargs["selected_agent_ids"] is None:
            select_ais = np.arange(self.nagents).tolist()
        else:
            select_ais = kwargs["selected_agent_ids"]
        ...
        for a_i, (neqs, niqs), log_pi, (peqs, piqs) in zip(
            ...
            #Aowabin editing script
            #addiing if statement
            if (a_i + 1) in select_ais:
    ...
    def update_policies(self, sample, soft=True, logger=None,
        ...
        #Aowabin to add block for running beta
        beta_running = kwargs["beta"] if "beta" in kwargs else self.beta

        #add block of code to isolate
        #troubleshoot
        #add selected_ids
        if "selected_agent_ids" not in kwargs:
            select_ais = np.arange(self.nagents).tolist()
        ...
        for a_i, probs, log_pi, pol_regs, ((eqs, iqs), (all_eqs, all_iqs)) in zip(
            ...
            if self.sep_extr_head:
                ...
                pol_target = q - v
                if a_i not in select_ais: #aowabin adding this step
                    pol_loss += 0.0
                if soft:
            ...
            for j in range(self.n_intr_rew_types):
                #print(f"The scaling for agent {a_i} and head {j} is {eqs[j]/iqs[j]} ")
                #changing to beta running
                q = eqs[j] + beta_running * iqs[j]
                all_q = all_eqs[j] + beta_running * all_iqs[j]
            ...
            if (a_i + 1) in select_ais:
                ...
                ##add the advantage function
                advantage = (-pol_target).detach().mean()
    ...
    @classmethod
    def init_adv(cls, file_coop, file_adv=None, load_critic=False, rogue_agent_ids=None):
        """
        Instantiate instance
        """
        coop_dict = torch.load(file_coop)
        instance = cls(**coop_dict['init_dict'])
        instance.init_dict = coop_dict['init_dict']

        if file_adv is not None:
            adv_dict = torch.load(file_adv)
            for i, (a, params, adv_params) in enumerate(zip(instance.agents, coop_dict['agent_params'], adv_dict["agent_params"])):
                if rogue_agent_ids is not None and i+1 not in rogue_agent_ids:
                    a.load_params(params)
                else:
                    a.load_params(adv_params)
        else:
            for a, params in zip(instance.agents, coop_dict['agent_params']):
                a.load_params(params)

        head_selector_params = coop_dict['head_selector_params']
        instance.head_selector.load_state_dict(head_selector_params['head_selector'])
        instance.head_selector_optimizer.load_state_dict(head_selector_params['head_selector_optimizer'])
        if load_critic:
            critic_params = coop_dict['critic_params']
            instance.critic.load_state_dict(critic_params['critic'])
            instance.target_critic.load_state_dict(critic_params['target_critic'])
            instance.critic_optimizer.load_state_dict(critic_params['critic_optimizer'])
        return instance
